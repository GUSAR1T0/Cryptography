...
public class DataEncryptionStandardTest {
    ...
    public static Collection<Object[]> data() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            data[i][2] = new String[][]{ConverterUtils.toBinaryStringBlocks(plaintextList.get(i), DataEncryptionStandardConstants.SIZE_OF_BLOCKS),
                    ciphertextList.get(i).split(StringUtils.divideOnBlocksPattern(DataEncryptionStandardConstants.SIZE_OF_BLOCKS))};
            data[i][3] = new String[]{ConverterUtils.toBinaryString(keyList.get(i)),
                    ConverterUtils.toBinaryString(keyList.get(i))};
            data[i][4] = new String[][]{ciphertextList.get(i).split(StringUtils.divideOnBlocksPattern(DataEncryptionStandardConstants.SIZE_OF_BLOCKS)),
                    ConverterUtils.toBinaryStringBlocks(plaintextList.get(i), DataEncryptionStandardConstants.SIZE_OF_BLOCKS)};
        }
    ...
    private void cipherTest(Cipher cipher, String input, String key, String[] binaryInputBlocks, String binaryKeyBlock,
        ...
        Assert.assertTrue("Another text input is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.TEXT_INPUT, ConverterUtils.toTextString(binaryInputBlocks))));
        Assert.assertTrue("Another binary input is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.BINARY_INPUT, Arrays.stream(binaryInputBlocks).collect(Collectors.joining()))));
        Assert.assertTrue("Another hex input is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.HEX_INPUT, ConverterUtils.toHexString(binaryInputBlocks))));
        Assert.assertTrue("Another text key is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.TEXT_KEY, ConverterUtils.toTextString(binaryKeyBlock))));
        Assert.assertTrue("Another binary key is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.BINARY_KEY, binaryKeyBlock)));
        Assert.assertTrue("Another hex key is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.HEX_KEY, ConverterUtils.toHexString(binaryKeyBlock))));
        Assert.assertTrue("Another text output is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.TEXT_OUTPUT, ConverterUtils.toTextString(binaryOutputBlocks))));
        Assert.assertTrue("Another binary output is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.BINARY_OUTPUT, Arrays.stream(binaryOutputBlocks).collect(Collectors.joining()))));
        Assert.assertTrue("Another hex output is expected",
                result.contains(createExpectedLine(DataEncryptionStandardResultType.HEX_OUTPUT, ConverterUtils.toHexString(binaryOutputBlocks))));
    }
